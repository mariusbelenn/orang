...
const initialInventory = [
  { id: 1, fruit: "Mango",  qty: 500, unit: "kg"  },
  { id: 2, fruit: "Banana", qty: 300, unit: "kg"  },
  { id: 3, fruit: "Apple",  qty: 200, unit: "box" },
  { id: 4, fruit: "Orange", qty: 150, unit: "kg"  },
];

const initialTransactions = [
  { id: 1, type: "IN",  fruit: "Mango",  qty: 200, unit: "kg",  date: "2026-03-28", note: "",             collector: "Juan dela Cruz", deliveredBy: "Pedro Reyes", supplier: "Santos Farm",    vehicle: "",          consumer: "" },
  { id: 2, type: "OUT", fruit: "Banana", qty: 50,  unit: "kg",  date: "2026-03-29", note: "Regular order", collector: "Maria Santos",   deliveredBy: "Tony Lim",    supplier: "",               vehicle: "Multicab",  consumer: "Market Stall A" },
  { id: 3, type: "IN",  fruit: "Apple",  qty: 100, unit: "box", date: "2026-03-30", note: "Fresh batch",   collector: "Ben Ramos",      deliveredBy: "Alex Cruz",   supplier: "Benguet Traders",vehicle: "",          consumer: "" },
  { id: 4, type: "OUT", fruit: "Mango",  qty: 80,  unit: "kg",  date: "2026-04-01", note: "",             collector: "Juan dela Cruz", deliveredBy: "Pedro Reyes", supplier: "",               vehicle: "Truck",     consumer: "Grocery Plus" },
];
...
export default function App() {
  ...
  const [customUnits,  setCustomUnits,         ]  = useLocalStorage("ft_customUnits",  []);
  ...
  // collect any custom units seen across forms so the selector remembers them
  ...
  function handleStockIn(e) {
    ...
    if (idx >= 0) {
      newInv[idx] = { ...newInv[idx], qty: newInv[idx].qty + qty, unit };
    } else {
      newInv.push({ id: invNextId, fruit: fruitName, qty, unit });
      setInvNextId(n => n + 1);
    }
    setInventory(newInv);
    setTx(t => [{ id: txNextId, type:"IN", fruit: fruitName, qty, unit, date: inForm.date, supplier: inForm.supplier, collector: inForm.collector, deliveredBy: inForm.deliveredBy, vehicle:"", consumer:"", note: inForm.note }, ...t]);
    setTxNextId(n => n + 1);
    setInForm(emptyIn());
  ...
  function handleStockOut(e) {
    ...
    let newInv = [...inventory];
    newInv[idx] = { ...newInv[idx], qty: newInv[idx].qty - qty };
    if (newInv[idx].qty === 0) newInv.splice(idx, 1);
    setInventory(newInv);
    setTx(t => [{ id: txNextId, type:"OUT", fruit: fruitName, qty, unit, date: outForm.date, supplier:"", collector: outForm.collector, deliveredBy: outForm.deliveredBy, vehicle: outForm.vehicle, consumer: outForm.consumer, note: outForm.note }, ...t]);
    setTxNextId(n => n + 1);
    setOutForm(emptyOut());
  ...
  function handleDeleteInv(id) { setInventory(inv => inv.filter(i => i.id !== id)); setDeleteInvModal(null); showToast("Item removed from inventory."); }
  ...
  const allVehicles  = [...new Set(transactions.map(t => t.vehicle).filter(Boolean))].sort();
        ...
        {deleteInvModal && (
          <div style={{ position:"fixed", inset:0, background:"#0007", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <div style={{ background:"#fff", borderRadius:14, padding:34, minWidth:300, boxShadow:"0 8px 40px #0005", textAlign:"center" }}>
              ...
              <div style={{ color:"#666", marginBottom:22, fontSize:13 }}>This will permanently remove this item from stock.</div>
              <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
                ...
                <form onSubmit={handleStockIn} style={{ ...card, display:"flex", flexDirection:"column", gap:16 }}>
                  ...
                  <div style={{ background:"#f0fff4", borderRadius:10, padding:"14px 16px", border:"1px dashed #82dbb0" }}>
                    ...
                    <div style={{ ...fRow, marginTop:12 }}>
                      ...
                      <UnitSelector value={inForm.unit} onChange={u => { setInForm({...inForm, unit:u}); addCustomUnit(u); }} customUnits={customUnits}/>
                  ...
                  <div style={{ background:"#fff8f0", borderRadius:10, padding:"14px 16px", border:"1px dashed #f4c97a" }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                      ...
                      <div style={fRow}>
                        ...
                        {outForm.fruit && outForm.qty && (() => { const inv = inventory.find(i => i.fruit === outForm.fruit); return inv && parseFloat(outForm.qty) > inv.qty ? <span style={{ color:"#c84b00", fontSize:11 }}>⚠ Exceeds stock ({inv.qty} {inv.unit||"kg"})</span> : null; })()}
                      </div>
                    </div>
                    <div style={{ ...fRow, marginTop:12 }}>
                      <label style={fLabel}>Unit / Measurement</label>
                      <UnitSelector value={outForm.unit} onChange={u => { setOutForm({...outForm, unit:u}); addCustomUnit(u); }} customUnits={customUnits}/>
                    </div>
                  </div>
